...
--2
--2.1
GRANT SELECT,INSERT,UPDATE ON ELEARN_APP_ADMIN.COURSE TO
ELEARN_professor1;
...
message varchar2(200), student_id number(4),
feedback_date date);
CREATE TABLE ELEARN_assistant3.FEEDBACK
( id number(6) primary key,
message varchar2(200), student_id number(4),
feedback_date date);
...
FROM ELEARN_assistant3.FEEDBACK;
...
INSERT INTO ELEARN_assistant3.FEEDBACK
VALUES(SYSDATE - TO_DATE('2000-01-01', 'yyyy-mm-dd'),
:NEW.MESSAGE,101,SYSDATE);
END IF;
END;
...
VARCHAR2 default 'advertisement', key3 VARCHAR2 default
'publicity') authid current_user AS
BEGIN
...
insert into homework values(1, 'Recap: Solve the following ...', 1, sysdate + 3, 0.25);
insert into homework values(2, 'Intro: Develop a program which ...', 1, sysdate + 6, 0.5);
commit;
...
--Method 3
create role select_homework;
grant select on homework to select_homework;
grant select_homework to elearn_professor1,elearn_professor2, elearn_assistant3; 

revoke select_homework from elearn_assistant3;
...
-- The select privilege is needed in order to make the "update" privilege work
grant select on homework to elearn_assistant3;
...
grant update(deadline) on homework to select_homework;
grant select_homework to elearn_assistant3;
...
create or replace procedure proc_marking(pStudId number, pHomeworkId number, pEvaluatorId number, pGrade number)
is
  vUpload number(1) := -1;
  vChecked number(1) := -1;
begin
    ...
    if (vUpload = 0) then
        ...
        if (vChecked >= 1) then 
            ...
            update solves s
            set grade = pGrade, evaluator_id = pEvaluatorId
            where s.student_id = pStudId
            and s.homework_id = pHomeworkId;
            commit;
        end if;
...
insert into user_ values (1, 'STUDENT', 'Name2', 'Firstname2', 'ELEARN_STUDENT2', sysdate - 300, null);   
insert into user_ values (2, 'STUDENT', 'Name3', 'Firstname3', 'ELEARN_STUDENT3', sysdate - 1000, null);
...
create or replace procedure homework_privs_student is
    cursor c is select username, year_study
                from user_ u join student s on (u.id = s.id)
                where upper(type_) = 'STUDENT';
begin
    for v_rec in c loop
...
execute homework_privs_student;
